...
import { factoryAnimals, factoryPeople } from "./factory.ts"; // Assicurati di avere questo file o crea i dati fittizi.
...
const generateMockAnimalDataForDb = (db: Database) => {
    factoryAnimals.forEach((animal) => {
        const stmt = db.prepare("INSERT INTO pets(id, owner, animal) VALUES (?, ?, ?);");
        stmt.run(animal.id, animal.owner, animal.animal);
    });
};
...
const generateMockPersonalDataForDb = (db: Database) => {
    factoryPeople.forEach((person) => {
        ...
        const stmt = db.prepare("INSERT INTO people(id, firstName, lastName, email, password, salt) VALUES (?, ?, ?, ?, ?, ?);");
        stmt.run(person.id, person.firstName, person.lastName, person.email, hashedPassword, salt);
    });
};
